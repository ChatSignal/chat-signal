# CWS Store Listing Copy

Copy-paste reference for Chrome Web Store dashboard. Keep in sync with actual CWS submission.

---

## ITEM SUMMARY

```
See what live chat is really saying. AI-powered message clustering, sentiment, and trending topics. 100% local.
```

Character count: 112 (limit: 132)

---

## ITEM DESCRIPTION

See what live chat is really saying — without losing it in the scroll.

Chat Signal analyzes YouTube and Twitch live chat in real-time and shows you:

- **Questions people are asking** — surfaced instantly so nothing gets buried
- **Chat mood** — is the room excited, positive, confused, or frustrated?
- **Trending topics** — the words and emotes that keep coming up
- **Session history** — review what chat talked about after the stream ends

**For creators and moderators:** Spot audience questions in real time, catch sentiment shifts early, and get an AI-generated summary of what happened during the stream.

Free. No account needed. All processing happens locally in your browser — no chat data ever leaves your computer. A small encoder model (~23MB) is downloaded once on first use for smarter clustering; the optional AI summary model (~400MB) downloads only if you opt in.

Works with YouTube and Twitch live streams.

---

## Trademark Notes

- Product name is "Chat Signal" — do NOT incorporate "YouTube" or "Twitch" into the product name
- Approved patterns: "works with YouTube", "for YouTube", "compatible with Twitch", "for Twitch"
- Prohibited: combining platform names with "extension" or "analyzer" in product name or description
- See: developers.google.com/youtube/terms/branding-guidelines and legal.twitch.com/legal/trademark/
